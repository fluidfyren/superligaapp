...
def bar_plot_teams2(p, team_list, teams, runde):
...
def bar_plot_teams_shape(p, team_list, teams, runde):
    points1 = np.zeros(len(team_list))
    points2 = np.zeros(len(team_list))
    points3 = np.zeros(len(team_list))
    ...
    for i, team in enumerate(teams):
        points1[i] = teams[team]['df']['Opponent Shape'][runde+0]
        points2[i] = teams[team]['df']['Opponent Shape'][runde+1]
        points3[i] = teams[team]['df']['Opponent Shape'][runde+2]
        modstander1.append(teams[team]['df']['Opponent'][runde+0])
        modstander2.append(teams[team]['df']['Opponent'][runde+1])
        modstander3.append(teams[team]['df']['Opponent'][runde+2])
    ...
    p.vbar(x=dodge('teams', -0.30, range=p.x_range), top='runde1', width=0.15, source=source,
       color="#c9d9d3", legend_label=f"Form for modstander i næste runde {runde+1}")
    
    p.vbar(x=dodge('teams', -0.1, range=p.x_range), top='runde2', width=0.15, source=source,
       color="#718dbf", legend_label=f"Form for modstander i næste runde {runde+2}")
    
    p.vbar(x=dodge('teams', +0.1, range=p.x_range), top='runde3', width=0.15, source=source,
       color="#e84d60", legend_label=f"Form for modstander i næste runde {runde+3}")
    
    p.vbar(x=dodge('teams', +0.3, range=p.x_range), top='middel', width=0.15, source=source,
       color="black", legend_label="Mean")
    
    p.add_tools(HoverTool(tooltips=[("Team", "@teams"), ("Modstander 1", "@modstander1 @runde1"),("Modstander 2", "@modstander2 @runde2"),("Modstander 3", "@modstander3 @runde3")]))
    p.x_range.range_padding = 0.1
    p.xaxis.major_label_orientation = 0.5
    p.xgrid.grid_line_color = None    
